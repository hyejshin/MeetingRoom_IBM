...
public class Home extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("admin") == null){
			...
			}else{ //key값 있으면 각 사이트로 이동(일반사용자) - 세션 설정
				session.setMaxInactiveInterval(10*60*60);
				session.setAttribute("admin", "no");
		        session.setAttribute("project", project);
			} // project가 master라면 모든 사이트를 다 가져와서 proj에 저장
		}else if(session.getAttribute("project").equals("master")){
			ArrayList<String> proj = adao.Projlist(); 
	        request.setAttribute("proj", proj);
		}else if(session.getAttribute("admin") == "yes") { 
			//System.out.println("admin yes!!__project : "+session.getAttribute("project"));
			nextPage = "AdminRsv.jsp";
		}
